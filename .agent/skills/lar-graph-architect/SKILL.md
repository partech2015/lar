...
## 4. 经典图架构范式 (Canonical Workflow Patterns)
根据 `examples/patterns` 目录，你必须熟练拼装以下高级基础流水线：
- **扇出与归并评判 (Fan-Out / Fan-In 模式)**：遇到 A/B 测试或多方案征选时，使用一层 `BatchNode` 挂载多个变种提示词节点并行生成，然后将 `next_node` 指向一个裁判 `LLMNode`，根据 `state` 中汇总的多个结果输出最终决断。
- **状态快照与断点续写 (Resumable Graph)**：对于极耗时操作，务必教导用户利用纯字典特性 `json.dump(state.get_all())` 保存快照。当环境崩溃后，通过注入读取的 JSON 字典给 `GraphExecutor.run_step_by_step()` 即可实现完美时光回溯，无需重新烧钱推理。
- **多智能体击鼓传花 (Handoff Pattern)**：各司其职的 Agent 之间不互相嵌套，而是通过 `ToolNode` 修改 `state.get("next_agent_id")`，配合顶层大 `Router` 路由流转，实现模块间的平滑接力。
